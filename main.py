"""Aggregates Montana-located listings from every taildraggers.com
...
ones whose location ends in ", MT".
...
Minnesota/Mississippi/Missouri repos' approach, with the state filter swapped
to "MT".
"""
...
PAGE_TITLE = "Taildragger Ads in Montana"
STATE_ABBR = "MT"
...
_MONTANA_LOCATION_RE = re.compile(r",\s*MT\s*$", re.IGNORECASE)


def _is_montana(location: str) -> bool:
    return bool(_MONTANA_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        montana_rows = [row for row in rows if _is_montana(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(montana_rows)} in Montana")
        all_listings.extend(montana_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Montana listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Montana listings: {len(listings)}")
