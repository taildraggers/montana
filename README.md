...
Daily aggregator of Montana-located taildragger classified listings,
...
repos' approach, with the state filter swapped to "MT".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, MT` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Montana"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/montana/`
...
```html
<iframe
  src="https://taildraggers.github.io/montana/"
  title="Taildragger Ads in Montana"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_MONTANA_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
